...
function Modal({ kind, close, data, setData, notify, settings, editingPantry, editingRecipe, onScanned }: { kind: Exclude<ModalKind, null>; close: () => void; data: AppData; setData: React.Dispatch<React.SetStateAction<AppData>>; notify: (s: string) => void; settings: SiteSettings; editingPantry: PantryItem | null; editingRecipe: Recipe | null; onScanned: (code: string) => void }) {
  ...
  const submit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (kind === 'pantry') {
      ...
      const ingredients = recipeIngredients.map(ingredient => ({ name: ingredient.name.trim(), amount: ingredient.amount.trim() || t('modal.toTaste') })).filter(ingredient => ingredient.name)
...
function RecipeIngredientEditor({ pantry, ingredients, setIngredients }: { pantry: PantryItem[]; ingredients: Recipe['ingredients']; setIngredients: Dispatch<SetStateAction<Recipe['ingredients']>> }) {
  ...
  return <section className="recipe-ingredients-editor"><div className="recipe-ingredients-head"><div><strong>{t('ingredients.title')}</strong><small>{t('ingredients.hint')}</small></div><button type="button" className="secondary" onClick={() => setIngredients(current => [...current, { name: '', amount: '' }])}><Plus size={15} />{t('ingredients.addRow')}</button></div>{pantryItems.length > 0 && <div className="pantry-ingredient-picks">{pantryItems.map(item => <button type="button" key={item.id} onClick={() => addFromPantry(item)}><Plus size={12} />{item.name}<small>{item.quantity} {item.unit}</small></button>)}</div>}<datalist id="recipe-pantry-items">{pantryItems.map(item => <option value={item.name} key={item.id} />)}</datalist><div className="recipe-ingredient-rows">{ingredients.map((ingredient, index) => <div className="recipe-ingredient-row" key={index}><input list="recipe-pantry-items" value={ingredient.name} onChange={event => update(index, 'name', event.target.value)} placeholder={t('ingredients.item')} required={index === 0} aria-label={`${t('ingredients.item')} ${index + 1}`} /><input value={ingredient.amount} onChange={event => update(index, 'amount', event.target.value)} placeholder={t('ingredients.amountPlaceholder')} aria-label={t('ingredients.amount', { count: index + 1 })} /><button type="button" className="icon-btn" onClick={() => remove(index)} aria-label={t('ingredients.remove', { count: index + 1 })}><X size={15} /></button></div>)}</div></section>
}
