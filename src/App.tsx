...
function NutritionSummary({ nutrition, grams, onGrams }: { nutrition: NutritionValues; grams: number; onGrams: (grams: number) => void }) { return <div className="nutrition-summary"><label><span>Porce</span><span className="number-with-unit"><input type="number" min="0" step="1" value={grams} onChange={event => onGrams(Number(event.target.value))} aria-label="Množství pro přepočet makroživin" /><span>g</span></span></label><div>{nutritionLabels.map(item => <span key={item.key}><small>{item.label}</small><strong>{nutritionValue(nutrition[item.key], grams)}{item.unit === 'g' ? ' g' : ''}</strong></span>)}</div></div> }
