...
function SettingsPage({ settings, setSettings, setCurrency, notify, changeLanguage }: { settings: SiteSettings; setSettings: React.Dispatch<React.SetStateAction<SiteSettings>>; setCurrency: (currency: Currency) => void; notify: (text: string) => void; changeLanguage: (language: Language) => void }) {
  ...
  return <><PageIntro title={t('settings.title')} subtitle={t('settings.subtitle')} button={t('settings.reset')} icon={<Settings />} onClick={reset} />
    <div className="settings-grid">
      ...
      <div className="app-version"><Info size={16} /><span>{t('settings.version')}</span><strong>{__APP_VERSION__}</strong></div>
    </div>
  </>
}
...
function Shopping({ data, settings, currency, rate, setData, money, open, notify }: { data: AppData; settings: SiteSettings; currency: Currency; rate: number; setData: React.Dispatch<React.SetStateAction<AppData>>; money: (n: number) => string; open: (m: ModalKind) => void; notify: (s: string) => void }) {
  ...
  const importReceiptItems = (items: Array<Pick<ShoppingItem, 'name' | 'quantity' | 'unit' | 'priceCzk' | 'barcode'>>) => {
    setData(current => ({ ...current, pantry: [...current.pantry, ...items.map<PantryItem>(item => { const product = current.products.find(candidate => candidate.ean === item.barcode) ?? current.products.find(candidate => candidate.name.toLocaleLowerCase() === item.name.toLocaleLowerCase()); return { id: uid(), name: item.name, category: product?.category || settings.defaultCategory, location: settings.defaultLocation, quantity: item.quantity, minimum: 0, unit: item.unit, priceCzk: item.priceCzk ?? product?.priceCzk ?? 0, purchasedAt: today(), productId: product?.id, barcode: item.barcode || product?.ean, image: product?.image, nutritionPer100g: product?.nutritionPer100g, portionGrams: product?.packageGrams } })] }))
  ...
  return <>
    ...
    {editingItem && <ShoppingItemDialog item={editingItem} currency={currency} rate={rate} close={() => setEditingItem(null)} save={updateItem} />}
  </>
}

function ShoppingItemDialog({ item, currency, rate, close, save }: { item: ShoppingItem; currency: Currency; rate: number; close: () => void; save: (item: ShoppingItem) => void }) {
  const { locale, t } = useI18n()
  const estimateCurrency: Currency = currency === 'CZK' ? 'EUR' : 'CZK'
  const [estimatedPrice, setEstimatedPrice] = useState(item.priceCzk === undefined ? '' : formatPriceInput(convertPrice(item.priceCzk, 'CZK', estimateCurrency, rate), locale))
  return <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><div className="modal"><div className="modal-head"><div><span className="eyebrow">{t('shopping.title')}</span><h2>{t('modal.editItem')}</h2></div><button className="icon-btn" onClick={close} aria-label={t('common.close')}><X size={20} /></button></div><form onSubmit={event => { event.preventDefault(); const form = new FormData(event.currentTarget); const parsedPrice = parsePriceInput(estimatedPrice); const minimum = String(form.get('minimum')).trim(); save({ ...item, name: String(form.get('name')).trim(), quantity: Number(form.get('quantity')), quantitySpecified: true, unit: String(form.get('unit')) as Unit, priceCzk: estimatedPrice.trim() && Number.isFinite(parsedPrice) ? convertPrice(parsedPrice, estimateCurrency, 'CZK', rate) : undefined, kanbanMinimum: minimum ? Number(minimum) : undefined, addToPantry: form.get('addToPantry') === 'on' }) }}>
    <Field label={t('modal.name')}><input name="name" required autoFocus defaultValue={item.name} placeholder={t('modal.whatBuy')} /></Field>
    <div className="form-grid"><Field label={t('settings.quantity')}><input name="quantity" type="number" min="0" step="0.1" defaultValue={item.quantity} required /></Field><Field label={t('settings.unit')}><UnitSelect defaultValue={item.unit} /></Field><Field label={t('modal.estimatedPriceCurrency', { currency: estimateCurrency })}><input name="price" type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]{0,2}" value={estimatedPrice} onChange={event => { if (/^\d*(?:[.,]\d{0,2})?$/.test(event.target.value)) setEstimatedPrice(event.target.value) }} /></Field><Field label={t('settings.minimum')}><input name="minimum" type="number" min="0" step="1" defaultValue={item.kanbanMinimum} /></Field></div>
...
type ReceiptDraft = Pick<ShoppingItem, 'name' | 'quantity' | 'unit' | 'priceCzk' | 'barcode'> & { id: string; selected: boolean }

function ReceiptImportDialog({ products, close, save }: { products: FoodProduct[]; close: () => void; save: (items: Array<Pick<ShoppingItem, 'name' | 'quantity' | 'unit' | 'priceCzk' | 'barcode'>>) => void }) {
  const { t } = useI18n()
  ...
  const recognize = async (file: File) => {
    ...
    try {
      ...
      const result = await response.json() as { items?: Array<{ name?: string; quantity?: number; unit?: string; priceCzk?: number; barcode?: string }>; error?: string }
      ...
      const recognized = (result.items || []).filter(item => item.name).map<ReceiptDraft>(item => {
        const product = products.find(candidate => candidate.ean === item.barcode) ?? products.find(candidate => candidate.name.toLocaleLowerCase() === item.name!.toLocaleLowerCase())
        return { id: uid(), selected: true, name: item.name!.trim(), quantity: Number(item.quantity) > 0 ? Number(item.quantity) : 1, unit: units.includes(item.unit as Unit) ? item.unit as Unit : 'ks', priceCzk: Number.isFinite(item.priceCzk) ? item.priceCzk : product?.priceCzk, barcode: item.barcode }
      })
      setItems(recognized)
